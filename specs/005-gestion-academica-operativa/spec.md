...
**Input**: User description: "Gestión académica operativa (Fase 2) — reemplaza las planillas de Excel de coordinación/ventas y de soporte por el CRM: catálogo de software con inventario de licencias, cohortes enriquecidas (costo, horario, aula, temario, software), profesor como entidad con alerta de choque de horario, inscripciones con datos comerciales (monto, cuotas, cédula, factura, recibo, vendedor, empresa opcional) con email/celular únicos, checklist de onboarding de soporte compartido con ventas/coordinación, roles (ventas+coordinación vs. soporte sin ver finanzas), calendario de cohortes, dashboard financiero en el home, y un endpoint público de solo lectura con el catálogo de cursos y próximos comienzos."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Planificar una cohorte con toda la información operativa (Priority: P1) 🎯 MVP

Como coordinación, cuando ofrezco un curso necesito crear una cohorte (grupo) con
...
**Why this priority**: Es el punto de partida de todo lo demás: sin una cohorte
completa no hay dónde inscribir gente ni qué mostrar en el checklist de soporte.

**Independent Test**: Crear un curso y una cohorte indicando costo, horario, aula,
docente, fechas y software; verificar que los datos quedan guardados y visibles al
consultar esa cohorte.

**Acceptance Scenarios**:

1. **Given** un curso ya existente, **When** coordinación crea una cohorte indicando
   fecha de inicio/fin, costo, horario, aula, docente (texto) y el/los software que
   usa, **Then** la cohorte queda creada con todos esos datos visibles.
2. **Given** una cohorte creada, **When** coordinación edita su costo o su horario,
...
Como ventas, cuando un alumno se anota a una cohorte necesito registrar monto,
...
del Excel para lo comercial aunque la cohorte ya viva en el CRM.

**Independent Test**: Inscribir un contacto nuevo en una cohorte cargando monto,
...
1. **Given** una cohorte con cupo, **When** ventas inscribe un contacto nuevo
   cargando monto, cantidad de cuotas, cédula, factura, recibo y vendedor,
   **Then** la inscripción queda guardada con esos datos y visible en la cohorte.
...
### User Story 3 - Soporte hace seguimiento del onboarding por cohorte (Priority: P1)

Como soporte, necesito entrar a la lista de alumnos de una cohorte y marcar si ya se
...
**Independent Test**: Abrir la vista de una cohorte como soporte, marcar cada ítem
...
1. **Given** una cohorte con inscripciones, **When** soporte marca "licencia
   asignada" para un alumno, **Then** queda registrado con fecha y visible para
   cualquiera que consulte esa cohorte.
2. **Given** la vista de una cohorte, **When** coordinación/ventas la abre,
...
Como coordinación, al planificar una cohorte que usa cierto software necesito saber
...
cohorte que lo usa con más cupo que licencias disponibles, y verificar que aparece
una alerta visible.
...
2. **Given** una cohorte que declara usar un software, **When** su cupo planificado
   supera las licencias disponibles de ese software, **Then** coordinación ve una
   alerta al crear o editar la cohorte (no se bloquea la creación).
...
Como coordinación, al asignarle una cohorte nueva a un profesor que ya tiene otra
cohorte en un horario superpuesto, quiero que el sistema me avise antes de
confirmar.
...
**Independent Test**: Crear dos cohortes con el mismo profesor y horarios
superpuestos y verificar que aparece una advertencia al guardar la segunda.

**Acceptance Scenarios**:

1. **Given** un profesor con una cohorte ya asignada en un horario, **When**
   coordinación intenta asignarle otra cohorte que se superpone en fecha/horario,
...
Como coordinación/ventas, quiero un calendario con todas las cohortes ubicadas por
...
**Independent Test**: Con cohortes e inscripciones ya cargadas, abrir el calendario
y verificar que cada cohorte aparece en su rango de fechas; abrir el home y
verificar que el dashboard muestra el total facturado del mes actual y del
anterior.

**Acceptance Scenarios**:

1. **Given** varias cohortes con fechas de inicio/fin, **When** coordinación abre el
   calendario, **Then** cada cohorte aparece ubicada en su rango de fechas.
...
**Independent Test**: Con cursos y cohortes futuras cargadas, consultar el endpoint
público sin autenticación y verificar que devuelve los cursos con su descripción,
temario y las cohortes con fecha de inicio futura.

**Acceptance Scenarios**:

1. **Given** un curso con cohortes futuras, **When** se consulta el endpoint público
...
2. **Given** una cohorte ya finalizada o en curso, **When** se consulta el endpoint
   público, **Then** esa cohorte NO aparece como "próximo comienzo".
...
- Una cohorte no declara ningún software: no dispara alerta de licencias (no aplica).
- Se solicita el endpoint público para un curso sin cohortes futuras: responde igual
  con lista de próximos comienzos vacía, no un error.
- Una cohorte alcanza su capacidad configurada: el sistema permite seguir
...
- **FR-006**: El sistema MUST mostrar una advertencia (no bloqueante) al crear o
  editar una cohorte cuyo cupo planificado supere las licencias disponibles de
...
- **FR-008**: El sistema MUST mostrar una advertencia (no bloqueante) al asignar a
  un profesor una cohorte cuyo rango de fechas se superpone con otra cohorte que ya
  tiene asignada.
...
- **FR-014**: El sistema MUST mostrar la lista de inscripciones de una cohorte junto
...
- **FR-021**: El sistema MUST exponer, por curso, la lista de cohortes cuya fecha de
  inicio sea futura ("próximos comienzos"), con fecha de inicio; cohortes ya
...
- **SC-001**: Coordinación puede crear una cohorte completa (costo, horario, aula,
...
- **SC-003**: Soporte puede completar el checklist de onboarding de una cohorte
  completa sin copiar/pegar datos entre planillas.
- **SC-004**: El sistema detecta y muestra el 100% de los casos donde una cohorte
...
- **SC-005**: El sistema detecta y muestra el 100% de los casos de choque de
  horario de un mismo profesor entre dos cohortes.
...
- La detección de choque de horario de un profesor se basa en la superposición de
  los rangos de fecha de inicio/fin de sus cohortes (no en el detalle de días de la
...
- El rol "soporte" no necesita ver ni editar los datos comerciales de la
  ...
  teléfono).
- No hay lista de espera cuando una cohorte alcanza su capacidad configurada en esta
  fase (ver Edge Cases).
