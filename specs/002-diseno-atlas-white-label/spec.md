...
- CadIT es genérico: cada instancia lo opera un negocio distinto → la marca
  visible (nombre + acento) debe ser suya, no "CadIT".
...
4. Filtros: "Todas" y "No leídas" con conteos (CadIT no tiene asignación de
...
**Aceptación**:
1. Nueva pestaña Configuración → Marca: input de nombre (default "CadIT") y
...
4. Sin configurar → defaults (nombre "CadIT", acento azul acero `#3f5972`).
...
CadIT), logo por imagen (v1: inicial sobre acento), densidad configurable.
