...
- [x] T002 Agregar prefijos de ID en `src/lib/db/ids.ts`: `installment: "inst"`,
      `payment: "pay"`
...
- [x] T003 Agregar `installment` y `payment` a `src/lib/db/schema.ts` per
...
- [x] T004 `pnpm db:generate` y revisar la migración A MANO. Debe ser
      puramente aditiva, sin backfill (DV-007).
- [x] T005 [P] Aplicar la migración contra una base efímera y verificar que
      corre limpia y es re-ejecutable (constitución IV). Receta en
      `quickstart.md`.
- [x] T006 Crear `src/server/billing.ts` con las funciones puras y sus firmas,
...
- [x] T007 [TEST] `tests/unit/billing-plan.test.ts`: la suma del plan debe
...
- [x] T008 Implementar `buildInstallmentPlan(amount, count, firstDueDate,
      everyMonths)` — función PURA, sin base, sin reloj propio.
- [x] T009 Implementar `createInstallmentPlan` y `replaceInstallmentPlan` en
      `billing.ts`, con validación FR-002 y `409` si ya hay pagos.
- [x] T010 `POST` y `GET /api/enrollments/[id]/installments` per el contrato,
      con `requireFullAccess`.
- [x] T011 [TEST] `tests/unit/billing-api.test.ts`: `soporte` recibe 403 sin
      tocar la base (mismo patrón que el test de `/api/dashboard/finance`).
- [x] T012 UI: sección "Plan de cuotas" en la ficha de inscripción, con el
...
- [x] T013 [TEST] `tests/unit/billing-payments.test.ts`: moneda distinta a la
      ...
      duplica.
- [x] T014 Implementar `recordPayment` con validación de moneda (FR-004),
      validación de FK contra la organización e idempotencia (FR-011).
- [x] T015 Implementar `voidPayment` con motivo y autor obligatorios; nunca
      borrado físico (FR-006).
- [x] T016 `POST /api/payments` y `POST /api/payments/[id]/void` per contrato.
- [x] T017 UI: registrar pago desde el estado de cuenta y desde el roster;
...
- [x] T018 [TEST] `tests/unit/billing-status.test.ts`: estados derivados con
      `hoy` inyectado — pendiente, parcial, pagada, vencida y el borde exacto
      del día de vencimiento.
- [x] T019 Implementar `installmentStatus(cuota, pagos, hoy)` — función pura.
- [x] T020 Implementar `listOverdue(organizationId, filtros, hoy)` con totales
      por moneda, nunca sumados entre sí.
- [x] T021 `GET /api/dashboard/overdue` per contrato, con `requireFullAccess`.
- [x] T022 UI: vista de morosidad, ordenada por días de atraso, filtrable por
      cohorte.
...
- [x] T023 [TEST] extender `tests/unit/finance.test.ts`: `collected` agrupa por
      moneda igual que lo facturado y excluye los pagos anulados.
- [x] T024 Implementar `collectedByCurrency` en `billing.ts`, reusando el
      criterio de rango de `sumByCurrencyInRange`.
- [x] T025 Extender `GET /api/dashboard/finance` con `collected` SIN cambiar la
      forma actual de la respuesta.
- [x] T026 UI: el panel del home muestra facturado y cobrado rotulados, por
...
- [x] T027 UI: estado de cuenta imprimible/exportable desde la ficha del
...
- [x] T028 Extender `scripts/e2e-selftest.mjs` con el escenario de cobranza:
...
- [x] T029 Gate técnico completo: `pnpm typecheck && pnpm lint && pnpm build &&
      pnpm test`, más `pnpm test:e2e` en verde.
- [x] T030 Actualizar `CLAUDE.md` (mapa del código: dónde se toca la cobranza)
