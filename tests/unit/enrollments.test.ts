...
describe("createEnrollment (005 US2)", () => {
  ...
  it("alta con contacto inline: crea el contacto y la inscripción", async () => {
    ...
    const result = await createEnrollment("org_1", {
      cohortId: "coh_1",
      contact: { name: "Diego", phone: "59899123456", email: "diego@example.com" },
      amount: 76000,
      installments: 12,
    });
    ...
    const contactValues = inserts[0]!.values as { name: string; email: string };
    expect(contactValues.name).toBe("Diego");
  ...
  it("duplicado de email/celular: el error del INSERT sube sin capturar (DV-002)", async () => {
    ...
    await expect(
      createEnrollment("org_1", {
        cohortId: "coh_1",
        contact: { name: "Otro", phone: "59899999999", email: "ya@existe.com" },
      })
